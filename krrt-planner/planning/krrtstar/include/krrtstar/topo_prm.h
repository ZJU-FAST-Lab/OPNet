...
public:
  ...
  enum NODE_STATE { NEW = 1, CLOSE = 2, OPENNING = 3 };
...
private:
  /* data */
  OccMap::Ptr occ_map_;  // environment representation
  ...
  /* collision check ray */
  void getCheckPos(const Eigen::Vector3d& pos, const Eigen::Vector3d& vel, 
                              const Eigen::Vector3d& acc, vector<Eigen::Vector3d>& grids, 
                              double hor_radius, double ver_radius);
  void getlineGrids(const Eigen::Vector3d& s_p, const Eigen::Vector3d& e_p, 
                               vector<Eigen::Vector3d>& grids);
  Eigen::Vector2d rotate90Clockwise2d(const Eigen::Vector2d& v);
  double hor_safe_radius_, ver_safe_radius_, replan_hor_safe_radius_, replan_ver_safe_radius_;
...
public:
  ...
  void setEnvironment(const OccMap::Ptr& env);
