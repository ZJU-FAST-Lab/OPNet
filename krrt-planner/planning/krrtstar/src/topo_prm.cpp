...
void TopologyPRM::init(ros::NodeHandle& nh) {
  ...
  offset_ = Eigen::Vector3d(0.5, 0.5, 0.5) - occ_map_->getOrigin() / resolution_;

  nh.param("topo_prm/replan_hor_safe_radius", replan_hor_safe_radius_, 0.3);
  nh.param("topo_prm/replan_ver_safe_radius", replan_ver_safe_radius_, 0.2);
...
list<GraphNode::Ptr> TopologyPRM::createGraph(Eigen::Vector3d start, Eigen::Vector3d end) {
  ...
  while (sample_time < max_sample_time_ && sample_num < max_sample_num_) {
    ...
    int pt_occ;
    Eigen::Vector3d grad;
    pt_occ = occ_map_->getVoxelState(pt);
    if (pt_occ != 0) {
  ...
  /* print record */
  std::cout << "[Topo]: sample num: " << sample_num << ", sample time: " << sample_time;

  std::cout << "[Topo]: node num before prune: " << graph_.size() << std::endl;
  pruneGraph();
  std::cout << "[Topo]: node num after prune: " << graph_.size() << std::endl;
...
bool TopologyPRM::lineVisib(const Eigen::Vector3d& p1, const Eigen::Vector3d& p2, double thresh,
  ...
  Eigen::Vector3d vel(p2 - p1), acc(0.0, 0.0, 0.0);
  ...
  while (casters_[caster_id].step(ray_pt)) {
    ...
    occ_map_->indexToPos(pt_id, pc);
    std::vector<Eigen::Vector3d> line_grids;
    getCheckPos(pc, vel, acc, line_grids, hor_safe_radius_, ver_safe_radius_);
    for (const auto& p : line_grids)
    {
      bool pt_occ = occ_map_->getVoxelState(p);
      if (pt_occ != 0) {
        occ_map_->indexToPos(pt_id, pc);
        return false;
      }
    ...
    // bool pt_occ = occ_map_->getVoxelState(pt_id);
    // if (pt_occ != 0) {
    //   occ_map_->indexToPos(pt_id, pc);
    //   return false;
    // }
...
void TopologyPRM::shortcutPath(vector<Eigen::Vector3d> path, int path_id, int iter_num) {
  ...
  for (int k = 0; k < iter_num; ++k) {
    ...
    for (int i = 2; i < dis_path.size(); ++i) {
      if (lineVisib(short_path.back(), dis_path[i], resolution_, colli_pt, path_id)) 
      {
        continue;
      }
      short_path.push_back(dis_path[i - 2]);
...
void TopologyPRM::setEnvironment(const OccMap::Ptr& env) { this->occ_map_ = env; }
...
void TopologyPRM::getCheckPos(const Eigen::Vector3d& pos, const Eigen::Vector3d& vel, 
                              const Eigen::Vector3d& acc, vector<Eigen::Vector3d>& grids, 
...
void TopologyPRM::getlineGrids(const Eigen::Vector3d& s_p, const Eigen::Vector3d& e_p, 
                               vector<Eigen::Vector3d>& grids)
{
  RayCaster raycaster;
  Eigen::Vector3d ray_pt;
...
inline Eigen::Vector2d TopologyPRM::rotate90Clockwise2d(const Eigen::Vector2d& v) 
{
  Eigen::Matrix2d r_m;
  r_m << 0, 1,
        -1, 0;
