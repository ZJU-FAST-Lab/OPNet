...
Eigen::Vector3d _initial_pos, _final_goal;
...
double total_len_pos_accu_(0.0);
double total_travel_time_(0.0);
bool start_track_(false);
ros::Time t_track_start_;
int emergency_trig_times_(0);
...
void collisionCallback(const std_msgs::Int8 count)
{
  collisions_ = count.data;
}
...
void odomCallbck(const nav_msgs::OdometryConstPtr odom)
{
  ...
  if (_first_odom)
  {
    ...
    _last_cmd.position.x = _initial_pos[0];
    _last_cmd.position.y = _initial_pos[1];
    _last_cmd.position.z = _initial_pos[2];
  }
...
//    return;
  ...
  {   
    ...
    if (t >= (_final_time - _start_time).toSec() - 0.01)
      _state = HOVER;
    ...
    for (int idx = 0; idx < _n_segment; ++idx)
    {
      ...
      { 
        ...
        Eigen::Vector3d last_pos(_last_cmd.position.x, _last_cmd.position.y, _last_cmd.position.z);
        total_len_pos_accu_ += (desire_pos - last_pos).norm();
  ...
  if ((_curr_posi - _final_goal).norm() <= 0.50) 
  {
    if (_state != HOVER)
    {
      total_travel_time_ = (ros::Time::now() - t_track_start_).toSec();
      std::ofstream oFile;
      oFile.open(_csv_path, std::ofstream::out | std::ofstream::app);
      ROS_WARN_STREAM("SUCCEED! WRITING TO FILE: "<<_csv_path);
      oFile << 2 << "," << total_travel_time_ << "," << total_len_pos_accu_ << "," << emergency_trig_times_ << "," << float(collisions_) << std::endl;
      oFile.close();
      _state = HOVER;
    }
  }
...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber collision_sub = node.subscribe("/occ_map/collisions", 1, collisionCallback);
  ...
  ros::Publisher goal_pub = node.advertise<quadrotor_msgs::PositionCommand>("/goal", 50);
  ros::Publisher final_goal_pub = node.advertise<geometry_msgs::PointStamped>("/final_goal", 10);
  node.param<std::string>("/traj_server_node/csv_path", _csv_path, "/home/wlz/catkin_ws/benchmark_result/with_pred/result.csv");
  ROS_WARN_STREAM("CSV PATH: "<<_csv_path);
  ...
  node.param("/traj_server_node/goal_x", goal_x, 0.0);
  node.param("/traj_server_node/goal_y", goal_y, 0.0);
  node.param("/traj_server_node/goal_z", goal_z, 0.0);

  // quadrotor_msgs::PositionCommand init_goal;
  // init_goal.position.x = init_x;
  // init_goal.position.y = init_y -0.5;
  // init_goal.position.z = init_z;
  // goal_pub.publish(init_goal);
  // ROS_INFO("GO TO FIRST INIT GOAL");
  ...
  // init_goal.position.z = init_z;
  // goal_pub.publish(init_goal);
  // ROS_INFO("GO TO SECOND INIT GOAL");
  ...
  final_goal.point.z = goal_z;
  _final_goal(0) = final_goal.point.x;
  _final_goal(1) = final_goal.point.y;
  _final_goal(2) = final_goal.point.z;
